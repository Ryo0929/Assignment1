...
public interface ItemRepository extends JpaRepository<Items, Integer> {
    ...
    @Query(value = "SELECT items.* FROM items WHERE items.item_category=?1 AND (keyword1 IN (?2,?3,?4,?5,?6) OR keyword2 IN (?2,?3,?4,?5,?6) or keyword3 IN (?2,?3,?4,?5,?6) or keyword4 IN (?2,?3,?4,?5,?6) or keyword5 IN (?2,?3,?4,?5,?6))",nativeQuery=true)
